 ...
 */
bool operator== (const SocketServer &s1, const SocketServer &s2);

/**
 *  Imprime la dirección y puerto en número con el formato:"dirección_ip:puerto"
 */
std::ostream& operator<<(std::ostream& os, const SocketServer& dt);
// -----------------------------------------------------------------------------
// -----------------------------------------------------------------------------
...
public:
     ...
     */
    SocketServer(const char * address, const char * port);

    /**
     *  Inicializa un Socket copiando los parámetros del socket
     */
    SocketServer(struct sockaddr * _sa, socklen_t _sa_len):sd(-1), sa(*_sa),
        sa_len(_sa_len){};
     ...
     */
    int recv(Message &obj, SocketServer * &sock);
     ...
     */
    int send(Message& obj, const SocketServer& sock);
    ...
    friend std::ostream& operator<<(std::ostream& os, const SocketServer& dt);

    friend bool operator== (const SocketServer &s1, const SocketServer &s2);
